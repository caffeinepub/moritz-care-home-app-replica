...
export default function AddNewResidentModal({
  ...
  const [physicians, setPhysicians] = useState<
    Array<{ id: string; name: string; specialty: string; contactInfo: string }>
  >([]);
  ...
  const handleAddPhysician = () => {
    setPhysicians([
      ...physicians,
      { id: crypto.randomUUID(), name: "", specialty: "", contactInfo: "" },
    ]);
  };
  ...
  const handleRemoveAdministrationTime = (
    medIndex: number,
    timeIndex: number,
  ) => {
    const updated = [...medications];
    updated[medIndex].administrationTimes = updated[
      medIndex
    ].administrationTimes.filter((_, i) => i !== timeIndex);
    setMedications(updated);
  };
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] dialog-solid-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the resident's information below. Fields marked with * are
            required.
          </DialogDescription>
        </DialogHeader>
                      ...
                      {medication.administrationTimes.map((time, timeIndex) => (
                        ...
                        >
                          <Input
                            ...
                            onChange={(e) => {
                              const updated = [...medications];
                              updated[medIndex].administrationTimes[timeIndex] =
                                e.target.value;
                              setMedications(updated);
                            }}
                          />
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleRemoveAdministrationTime(
                                medIndex,
                                timeIndex,
                              )
                            }

...
export default function PharmacyEditPage() {
  const { residentId, pharmacyId } = useParams({
    from: "/resident/$residentId/pharmacy/$pharmacyId/edit",
  });
  ...
  useEffect(() => {
    if (resident) {
      const pharmacy = resident.pharmacyInfos.find(
        (p) => p.id.toString() === pharmacyId,
      );
      if (pharmacy) {
        setFormData({
          name: pharmacy.name,
